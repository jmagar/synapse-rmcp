...
MCP_URL="${SYNAPSE_MCP_URL:-http://localhost:40080/mcp}"
...
Options:
  --url URL              MCP URL. Default: SYNAPSE_MCP_URL or http://localhost:40080/mcp.
  --token TOKEN          Expected static bearer token. Default: SYNAPSE_MCP_TOKEN.
...
if [[ -z "$TOKEN" ]]; then
  echo "ERROR: set SYNAPSE_MCP_TOKEN or pass --token" >&2
  exit 2
fi
...
http_code() {
  curl -sS --max-time "$TIMEOUT" -o "${BODY_FILE}" -w '%{http_code}' "$@"
}
...
expect_success_jsonrpc() {
  ...
  body_check="$(python3 - "${BODY_FILE}" <<'PY'
import json, sys
try:
