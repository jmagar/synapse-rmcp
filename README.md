...
The template ships Claude Code, Codex, and Gemini plugin surfaces from one shared `plugins/synapse2/` package. See [docs/PLUGINS.md](docs/PLUGINS.md) for the manifest layout, shared MCP config, skills, hook setup contract, and per-host adaptation checklist.
...
- **Both transports** — `synapse2 serve` (Streamable HTTP) and `synapse2 mcp` (stdio)
...
```
SynapseClient  (src/synapse2.rs)    ← HTTP/GraphQL/gRPC calls to upstream
      ↓
SynapseService (src/app.rs)        ← all business logic lives here
      ↓
  ┌──────────────────────────────────┐
...
The rule: **zero business logic in `tools.rs` or `cli.rs`**. Both are pure shims. All logic belongs in `app.rs` (or `synapse2.rs` for transport concerns). For business actions, MCP + CLI parity is mandatory; REST/Web are project-type dependent.

## Quickstart — run the stub

```bash
git clone https://github.com/jmagar/synapse2
cd synapse2
...
curl -s -X POST http://localhost:40060/mcp \
  ...
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"synapse2","arguments":{"action":"greet","name":"Alice"}}}'
```
...
```bash
git clone https://github.com/jmagar/synapse2 myservice-mcp
cd myservice-mcp
```
...
| Find | Replace with |
|------|-------------|
| `synapse2` | `myservice-mcp` (Cargo.toml package name) |
| `synapse2` (binary name) | `myservice` (Cargo.toml `[[bin]] name`) |
| `SynapseClient` | `MyServiceClient` |
| `SynapseService` | `MyServiceService` |
| `SynapseConfig` | `MyServiceConfig` |
| `SynapseRmcpServer` | `MyServiceRmcpServer` |
| `SYNAPSE_API_URL` | `MYSERVICE_API_URL` |
| `SYNAPSE_MCP_*` | `MYSERVICE_MCP_*` |
| `synapse:read` | `myservice:read` |
| `synapse://schema/mcp-tool` | `myservice://schema/mcp-tool` |

### 2. Replace SynapseClient with your API client

Edit `src/synapse2.rs`. This is the only file that makes network calls.
...
Edit `src/config.rs` to rename `SynapseConfig` fields and env var names. Edit `config.toml` and `.env.synapse2`.

## Command modes

```
synapse2 [serve]          Start Streamable HTTP MCP server (default)
synapse2 mcp              Start stdio MCP transport
synapse2 greet [--name]   CLI: greet
synapse2 echo --message   CLI: echo
synapse2 status           CLI: server status
synapse2 --help           Usage
synapse2 --version        Version
```

## MCP tool actions

The single `synapse2` tool dispatches on the `action` parameter:
...
Set `SYNAPSE_MCP_TOKEN`. All `/mcp` requests must include `Authorization: Bearer <token>`.

### No auth (loopback only)

Set `SYNAPSE_MCP_NO_AUTH=true` or bind to `127.*`. Only legal for local development.

### OAuth (Google)

Set `SYNAPSE_MCP_AUTH_MODE=oauth` and the OAuth env vars below. The server issues RS256 JWTs after Google authentication. OAuth and bearer can coexist (OAuth mode disables the static token by default; set `disable_static_token_with_oauth = false` to keep both active).
...
|----------|----------|---------|-------------|
| `SYNAPSE_API_URL` | no | — | Upstream service base URL |
| `SYNAPSE_API_KEY` | no | — | Upstream service API key |
| `SYNAPSE_MCP_HOST` | no | `0.0.0.0` | Bind host |
| `SYNAPSE_MCP_PORT` | no | `40060` | Bind port |
| `SYNAPSE_MCP_NO_AUTH` | no | `false` | Disable auth (loopback only; 1/true/yes) |
| `SYNAPSE_MCP_TOKEN` | no | — | Static bearer token for `/mcp` |
| `SYNAPSE_MCP_ALLOWED_HOSTS` | no | — | Extra comma-separated Host header values |
| `SYNAPSE_MCP_ALLOWED_ORIGINS` | no | — | Extra comma-separated CORS origins |
| `SYNAPSE_MCP_PUBLIC_URL` | OAuth | — | Public URL (e.g. `https://myservice.synapse2.com`) |
| `SYNAPSE_MCP_AUTH_MODE` | no | `bearer` | `bearer` or `oauth` |
| `SYNAPSE_MCP_GOOGLE_CLIENT_ID` | OAuth | — | Google OAuth client ID |
| `SYNAPSE_MCP_GOOGLE_CLIENT_SECRET` | OAuth | — | Google OAuth client secret |
| `SYNAPSE_MCP_AUTH_ADMIN_EMAIL` | OAuth | — | Admin email address |
...
just dev              # SYNAPSE_MCP_HOST=127.0.0.1 SYNAPSE_MCP_NO_AUTH=true cargo run -- serve mcp (loopback only, no auth)
...
This checklist covers everything you need to adapt synapse2 for a real service. Work through it top-to-bottom; each step is independent.

### Checklist

#### Core: rename and implement

1. **Replace all occurrences of `synapse2`/`Example`/`EXAMPLE` with your service name**
   ...
   | `SYNAPSE_API_URL` | `MYSERVICE_API_URL` |
   | `SYNAPSE_MCP_*` | `MYSERVICE_MCP_*` |
   | `SYNAPSE_NOAUTH` | `MYSERVICE_NOAUTH` |
   | `synapse:read` | `myservice:read` |
   | `synapse://schema/mcp-tool` | `myservice://schema/mcp-tool` |
   | `.synapse2` (data dir) | `.myservice` (in `config.rs` and `docker-compose.yml`) |

2. **Implement your API client in `src/synapse2.rs`**
...
3. **Add service methods to `src/app.rs`**
   ...
   Each public method on `SynapseService` corresponds to one MCP action. Business logic, caching, and retries go here — not in `tools.rs`.
...
6. **Update `src/config.rs`** with service-specific config fields
   ...
   Rename `SynapseConfig` and add any fields your service needs. Update env prefixes throughout.
...
8. **Update `config/Dockerfile` binary name, port, and cache IDs**
   ...
   Replace every occurrence of `synapse2` (binary copy, cache IDs, CMD, LABEL) with your binary name. Update `EXPOSE` to your port.

9. **Update `docker-compose.yml`**

   - Change `40060` to your service's port (must match `config.toml [mcp] port`)
   - The `${HOME}/.synapse2:/data` volume is already set; rename `.synapse2` to your service

10. **Update `entrypoint.sh`**

    Uncomment the `REQUIRED_VARS` check block and add your service's required env vars. Replace `SYNAPSE_API_KEY` references with your prefix.
...
14. **Update GitHub workflow files** (`.github/workflows/`)
    ...
    - `synapse2` → your repo name (cache keys)
    - `synapse2` → your Docker image name
    - `synapse2` → your binary name
    - `jmagar` → your GitHub org/username (image registry path)

15. **Update `.env.synapse2`** with your service's actual variable names and descriptions

16. **Update `config.synapse2.toml`** with your service's actual config fields

#### Plugin and skills

17. **Update plugin.json userConfig for your service's credentials**

    Edit `plugins/synapse2/.claude-plugin/plugin.json`. Replace the `example_api_url` / `example_api_key` fields with your service's actual credential names and descriptions.

18. **Update `plugins/synapse2/hooks/plugin-setup.sh`**

    Replace `SYNAPSE_*` env var names, `synapse2` service references, and add any service-specific credentials your binary needs.

19. **Update `plugins/synapse2/skills/`**

    Replace the action table in `plugins/synapse2/skills/synapse2/SKILL.md` with your actual actions and documented response shapes. Keep or adapt `plugins/synapse2/skills/scaffold-project/SKILL.md` if you want the elicitation setup wizard to generate approval-first scaffold plans. Good skill docs drive better AI tool use.

20. **Update `plugins/synapse2/.codex-plugin/plugin.json`** for Codex plugin registry
    ...
    See `plugins/synapse2/.codex-plugin/README.md` for the full field reference.
